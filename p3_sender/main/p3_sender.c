...
void uart_putchar(char c, uint8_t uartNum) { uart_write_bytes(uartNum, &c, 1); }
...
void initUarts(void) {
  ...
  // Set UART_RECEIVER
  ESP_ERROR_CHECK(
      uart_driver_install(UART_RECEIVER, RX_BUF_SIZE, RX_BUF_SIZE, 0, NULL, 0));
  ESP_ERROR_CHECK(uart_param_config(UART_RECEIVER, &uart_config));
  ESP_ERROR_CHECK(uart_set_pin(UART_RECEIVER, TXD_PIN, RXD_PIN,
                               UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
  // Set UART_SENDER
  ESP_ERROR_CHECK(
      uart_driver_install(UART_SENDER, RX_BUF_SIZE, RX_BUF_SIZE, 0, NULL, 0));
  ESP_ERROR_CHECK(uart_param_config(UART_SENDER, &uart_config));
  ESP_ERROR_CHECK(uart_set_pin(UART_SENDER, TXD_PIN_SEND, RXD_PIN_SEND,
                               UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
