...
static void bme280_task(void *pvParameters) {
    ...
    while (1) {
        ...
        //Transmits data as slave to the buffer for the master to read
        i2c_slave_write_buffer(I2C_SLAVE_NUM, (uint8_t *)&data, sizeof(data), 1000 / portTICK_PERIOD_MS);
...
static esp_err_t i2c_slave_init(){
    ...
    return i2c_driver_install(I2C_SLAVE_NUM, conf_slave.mode, BUFF_SIZE, BUFF_SIZE, 0);
}
...
static esp_err_t i2c_master_init() {
    ...
    return i2c_driver_install(I2C_MASTER_NUM, conf.mode, BUFF_SIZE, BUFF_SIZE, 0);
}
