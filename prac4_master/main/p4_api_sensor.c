...
void printRegs();
static esp_err_t i2c_read_esp32(uint8_t addr, uint8_t* header, uint8_t *data_rd, size_t size);
...
static void bme280_task(void *pvParameters) {
    ...
    while (1) {
        i2c_read_esp32(ESP32_SLAVE_ADDR, header, trans_data, sizeof(trans_data));
        ESP_LOGI(TAG, "Data: 0x%x 0x%x 0x%x", trans_data[0], trans_data[1], trans_data[2]);
...
static esp_err_t i2c_read_esp32(uint8_t addr, uint8_t* header, uint8_t *data_rd, size_t size) {
    ...
    i2c_master_write_byte(cmd, ESP32_SLAVE_ADDR << 1 | I2C_MASTER_WRITE, true);
    i2c_master_write_byte(cmd, addr, true);
    i2c_master_write(cmd, header, sizeof(header), true);
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, ESP32_SLAVE_ADDR << 1 | I2C_MASTER_READ, true);
    if (size > 1) {
        i2c_master_read(cmd, data_rd, size - 1, I2C_MASTER_ACK);
    }
    i2c_master_read_byte(cmd, data_rd + size - 1, I2C_MASTER_NACK); // Read the last byte with NACK
    i2c_master_stop(cmd);
